...
// Taxonomy attempt for patron types
  ...
  function uml_shortcode_faq_func($atts) {
    ...
    //print $q;
    $myfaqs = $wpdb_sp->get_results($wpdb_sp->prepare($q));
...
///////////////
  ...
  function get_schedule($get_date, $weekday) {
    ...
    $exceptions = $wpdb_sp->get_results($wpdb_sp->prepare("SELECT exception_name, exception_date, exception_schedule FROM schedule_exceptions WHERE exception_date = '" . $get_date . "' LIMIT 1"));
    ...
    if (!empty($exceptions)) {
      ...
      $get_hours = $wpdb_sp->get_results($wpdb_sp->prepare("$q"));
